...
/// <summary>
/// The closed reference list spec 6.5.4 requires for <see cref="Pupil.StateOfOrigin"/> and
/// <see cref="Pupil.Lga"/>: "Picked from the 36 states plus the Federal Capital Territory. Free text
/// is not accepted, because this field is reported on... The picker is filtered by the selected
/// state, which is the only way this field gets entered correctly."
/// </summary>
/// <remarks>
/// <para>
/// DATA-ACCURACY CAVEAT, disclosed rather than presented as authoritative: the 37 state names are a
/// well-known, low-risk fixed list; the 774 LGA names beneath them were compiled from general
/// knowledge with no authoritative source (e.g. the NPopC/INEC gazette) available to check against
/// in this environment. The mechanism (validate against a per-state closed list, never free text) is
/// exactly what 6.5.4 requires; the LGA data itself should be diffed against an authoritative source
/// before this is relied on for compliance-grade reporting — see <c>backend/docs/ASSUMPTIONS.md</c>
/// §2.27.
/// </para>
/// <para>
/// Both lookups are case-insensitive; <see cref="TryNormalizeState"/> and <see cref="TryNormalizeLga"/>
/// return the CANONICAL casing so the stored value is consistent regardless of how the client typed it.
/// </para>
/// </remarks>
public static class NigerianGeography
{
    /// <summary>The 36 states plus the FCT, in the order the National Assembly lists them.</summary>
    ...
    private static readonly Dictionary<string, string> StatesByKey =
        States.ToDictionary(state => state.ToLowerInvariant(), state => state);

    private static readonly Dictionary<string, IReadOnlyList<string>> LgasByState =
        new Dictionary<string, IReadOnlyList<string>>(StringComparer.Ordinal)
        {
            ...
            [
                ...
                "Takai", "Tarauni", "Tofa", "Tsanyawa", "Tudun Wada", "Ungogo", "Warawa",
                "Wudil",
            ],
            ["Katsina"] =
            [
    ...
    /// </summary>
    public static bool TryNormalizeState(string? rawState, out string canonicalState)
    {
        canonicalState = string.Empty;
        ...
        if (StatesByKey.TryGetValue(rawState.Trim().ToLowerInvariant(), out var match))
        {
    ...
    /// <summary>
    /// Attempts to resolve <paramref name="rawLga"/> against the LGA list for
    /// <paramref name="canonicalState"/> (must already be one of <see cref="States"/>'s canonical
    /// spellings — call <see cref="TryNormalizeState"/> first). Returns the canonical spelling on
    /// success.
    /// </summary>
    public static bool TryNormalizeLga(string canonicalState, string? rawLga, out string canonicalLga)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(canonicalState);

        canonicalLga = string.Empty;

        if (string.IsNullOrWhiteSpace(rawLga) || !LgasByState.TryGetValue(canonicalState, out var lgas))
        {
            return false;
        }

        var trimmed = rawLga.Trim();
        var match = lgas.FirstOrDefault(lga => string.Equals(lga, trimmed, StringComparison.OrdinalIgnoreCase));
