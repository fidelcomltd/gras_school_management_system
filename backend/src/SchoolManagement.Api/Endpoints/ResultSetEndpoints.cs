...
public sealed class ResultSetEndpoints : IEndpointModule
{
    ...
    private static void MapApprove(RouteGroupBuilder group) =>
        group.MapPost("/{resultSetId:guid}/approve", async (
            ...
            .RequirePrivilege(Privileges.Results.Approve, ScopeParameterKind.ResultSet, "resultSetId")
            .RequireCsrfToken()
            .RequireIdempotencyKey(required: false)
            .RequireRateLimiting(RateLimitingOptions.SensitivePolicyName)
            ...
            .WithDescription(
                "Spec 6.7.8/6.7.11 (TASK-0090): moves Awaiting Approval to Approved, writing " +
                "approvedAt/approvedBy. No body. 409 `result_set.not_awaiting_approval` from any " +
                "other state. 409 `result_set.needs_recompute` when a settings save since the last " +
                "computation has flagged this set (spec 6.2.9): \"Marks have changed since the last " +
                "computation. Run computation again before approving.\" An unknown id is 403, not " +
                "404 (TASK-0071 ruling). `Idempotency-Key` is ACCEPTED, not required.")
    ...
    private static void MapReturn(RouteGroupBuilder group) =>
        group.MapPost("/{resultSetId:guid}/return", async (
            ...
            .RequirePrivilege(Privileges.Results.Return, ScopeParameterKind.ResultSet, "resultSetId")
            .RequireCsrfToken()
            .RequireIdempotencyKey(required: false)
            .RequireRateLimiting(RateLimitingOptions.SensitivePolicyName)
            ...
            .WithDescription(
                "Spec 6.7.8/6.7.11 (TASK-0090): moves Awaiting Approval or Approved to Returned for " +
                "Correction, storing `reason` (trimmed, 10-500 characters, else 422) and reopening " +
                "marks for the class teacher. A set can be returned any number of times; each return " +
                "is its OWN audit event and overwrites any earlier reason. 409 " +
                "`result_set.not_returnable` from any other state. An unknown id is 403, not 404 " +
                "(TASK-0071 ruling). `Idempotency-Key` is ACCEPTED, not required.")
