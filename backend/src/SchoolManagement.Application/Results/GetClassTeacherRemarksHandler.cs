...
/// <summary>Handles <see cref="GetClassTeacherRemarksQuery"/>.</summary>
internal sealed class GetClassTeacherRemarksHandler(
    IArmRepository arms,
    ITermRepository terms,
    IEnrolmentRepository enrolments,
    IResultSetRepository resultSets,
    IPupilRemarkRepository pupilRemarks)
    : IRequestHandler<GetClassTeacherRemarksQuery, Result<RemarkSheetDto>>
{
    /// <inheritdoc />
    public async Task<Result<RemarkSheetDto>> HandleAsync(GetClassTeacherRemarksQuery request, CancellationToken cancellationToken)
    {
        ...
        IReadOnlyList<PupilRemarkSnapshot> remarks = resultSet is null
            ? []
            : await pupilRemarks.ListReadOnlyAsync(resultSet.Id, RemarkKind.ClassTeacher, cancellationToken).ConfigureAwait(false);
