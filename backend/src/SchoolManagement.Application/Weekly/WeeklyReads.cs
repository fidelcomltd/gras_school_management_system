    ...
    : IRequestHandler<GetWeeklyCompletionQuery, Result<WeeklyCompletionReportDto>>
{
    ...
    public async Task<Result<WeeklyCompletionReportDto>> HandleAsync(GetWeeklyCompletionQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            var onRoll = (await enrolments.ListActiveRosterByArmAsync(armId, cancellationToken).ConfigureAwait(false)).Count;
...
internal sealed class GetWeeklyIllnessHandler(
    ITermRepository terms, IArmRepository arms, IClassLevelRepository levels, IPupilRepository pupils, IWeeklyReportRepository weekly)
    : IRequestHandler<GetWeeklyIllnessQuery, Result<WeeklyIllnessReportDto>>
{
    ...
    public async Task<Result<WeeklyIllnessReportDto>> HandleAsync(GetWeeklyIllnessQuery request, CancellationToken cancellationToken)
    {
        ...
        foreach (var group in (await weekly.ListSymptomsAsync(term.Id, cancellationToken).ConfigureAwait(false))
            .GroupBy(entry => entry.PupilId).Where(group => group.Count() >= MinimumDays))
        {
            if (await pupils.FindReadOnlyByIdAsync(group.Key, cancellationToken).ConfigureAwait(false) is not { } pupil)
            {
                continue;
            }
...
/// <summary>Resolves admin account ids to staff names for the "last edited by" columns.</summary>
