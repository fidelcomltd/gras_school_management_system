    ...
    : DbContext(options)
{
    ...
    /// <summary>TASK-0072 stage 1. Internal, not public: only this assembly's repositories may query it.</summary>
    internal DbSet<RatingScale> RatingScales => Set<RatingScale>();

    /// <summary>TASK-0072 stage 1. Internal, not public: only this assembly's repositories may query it.</summary>
    internal DbSet<RatingScalePoint> RatingScalePoints => Set<RatingScalePoint>();

    /// <summary>TASK-0072 stage 2a. Internal, not public: only this assembly's repositories may query it.</summary>
    internal DbSet<DevelopmentDomain> DevelopmentDomains => Set<DevelopmentDomain>();

    /// <summary>TASK-0072 stage 2a. Internal, not public: only this assembly's repositories may query it.</summary>
    internal DbSet<DevelopmentIndicator> DevelopmentIndicators => Set<DevelopmentIndicator>();

    /// <summary>TASK-0072 stage 3b. Internal, not public: only this assembly's repositories may query it.</summary>
    internal DbSet<Trait> Traits => Set<Trait>();

    /// <summary>TASK-0072 stage 3b. Internal, not public: only this assembly's repositories may query it.</summary>
    internal DbSet<TraitBlock> TraitBlocks => Set<TraitBlock>();
    ...
    /// <summary>
    /// TASK-0086 stage A. Internal, not public: only this assembly's repositories may query it. One
    /// row per pupil per result set (spec 09 §6.7.3) — <c>timesAbsent</c> is derived, never stored
    /// (ruling A).
    /// </summary>
    internal DbSet<AttendanceEntry> AttendanceEntries => Set<AttendanceEntry>();

    /// <summary>
    /// TASK-0086 stage A. Internal, not public: only this assembly's repositories may query it. One
    /// row per pupil per <c>kind</c> per result set (spec 09 §6.7.3; appendix C.6).
    /// </summary>
    ...
    /// </summary>
    internal DbSet<DevelopmentRating> DevelopmentRatings => Set<DevelopmentRating>();
