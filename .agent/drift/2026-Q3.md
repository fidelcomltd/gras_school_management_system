...
- 2026-09-16 **`AuthLogRedactionTests.SignInAndChangePassword_NeverWriteThePlaintextPasswordToConsoleOutput`
  ...
  what fails, so the redaction assertions it guards are the thing at risk.** Surfaced by TASK-0065,
  caused by neither TASK-0065 nor the database: the test never touches `DatabaseAvailability` or
  `ApiTestFixture`'s container path. Found because TASK-0065 made a full local integration run cheap
  enough (4m37s) to actually perform, which is the card's value proposition working as designed.
