...
**Current: `ac7ed393aaa7464e926e6185437f5124c3f59cc0c7cb1c8e8dc47dec1c9928da`** · **110 paths** ·
**256 schemas** · api version `v1` · moved 2026-09-23 by the admission health override (§6.5.16).
...
- 2026-09-23 **Admissions slice 2 (stepped flow, spec 6.5.11 + 6.5.16 override) done**, branch `admissions-flow` (stacked on `admissions-register`): `/admissions/:id?step=n` resumes at the first blocking step; steps 2-8 reuse the record panels, step 8 PATCHes section I only (nulls = unchanged), step 9 shows completeness + summary + the approve form; the queue gains New admission and Continue; approval confirmation shows the number large with Copy. New privilege `pupil.admission.override` (register 96; Super Admin + Head Teacher; migration adds it to Head Teacher additively in SQL, never overwriting an edited role); `healthOverrideReason` waives ONLY `health.unanswered`, audited as its own event. Not built: admission slip print, the queue's per-row full missing list and last step reached, a leave-with-unsaved-changes guard. Next: bulk import.
