...
**Current: `c5dbaee99c82038bc063616071b21035028649951271e6bf3d5f3fc521b5f863`** · **121 paths** ·
**279 schemas** · api version `v1` · moved 2026-09-26 by pupil photographs and document scans (spec 6.5.4, 6.5.8) (then a description-only re-promote after review, structurally identical by `jq`).
...
**Status changes and transfers additive verified mechanically** (`jq`, `MSYS_NO_PATHCONV=1`, `tr -d '\r'`, description/example/examples stripped, non-null probe on `/pupils/{id}`, `PupilDto` and the new `/transfer`): +3 paths (`/pupils/{id}/status`, `/transfer`, `/enrolments`), +9 schemas (`ChangePupilStatusCommand`, `TransferPupilCommand`, `PupilMovement*`, `PupilEnrolment*`, `PupilStatusChangeDto`), 0 removed, 0 existing paths or schemas changed. `CONTRACT.lock`'s informational `dotnet.sdk` line now reads 10.0.401 (the SDK that ran the script from the repo root on `PC`); nothing checks it.

**Incomplete-records report additive verified mechanically** (same `jq` recipe, non-null probe): +1 path (`/reports/incomplete-records`), +3 schemas (`IncompleteRecord*Dto`), 0 removed, 0 existing paths or schemas changed.
...
**Admissions C-F/H additive verified mechanically** (`jq` against HEAD): +7 paths, +23 schemas; no existing path or schema changed beyond description text. **Corrected 2026-09-23: the weekly-reports check below was vacuous as first run** (Git Bash's MSYS path conversion rewrote `--arg p "/api/..."` into a Windows path, so every lookup was null on both sides); re-run with `MSYS_NO_PATHCONV=1` and a sanity probe against `739e48b`, it holds: nothing changed beyond description text.

**Weekly reports additive verified mechanically** (`jq` against HEAD): +8 paths, +21 schemas, zero existing paths or schemas changed.
...
- 2026-09-26 **Pupil photographs and document scans (spec 6.5.4, 6.5.8, 9.6) done**, branch `pupil-photos-documents` (human rulings same day: attaching a scan ticks an unticked row; scans are removable, audited, the tick stays; the browser downscales scans to 2000 px and photos to 800 px; replaced or removed bytes stay in Cloudinary, never deleted): `POST`/`DELETE`/`GET /pupils/{id}/photo` + `GET /photo/thumbnail` (3 MB with the spec's message; JPEG/PNG by magic bytes; 400x400 centre crop JPEG q80 + 96 px, EXIF applied then stripped, original discarded; audited `pupil.photo.update`/`pupil.photo.remove`; served through `pupil.view`), `POST`/`DELETE`/`GET /pupils/{id}/documents/{type}/file` (PDF/JPEG/PNG to 5 MB; images re-encoded, PDFs stored as `raw` Cloudinary resources; a row with a scan cannot be unticked); every decode refuses more than 25 megapixels from the header (decompression-bomb guard, logo/signature too); photograph joins the chased set and the report; additive migration `AddPupilPhotoAndDocumentFiles`; contract `c5dbaee9…`. **Not smoke-tested against real Cloudinary**: `cloudinary-smoke-test` now round-trips a PDF too, and needs a run with real keys.
- 2026-09-26 **Staging moved to Render's FREE plan** (human: no payment card; branch `deploy/staging-free`): `plan: free`; no Shell there, so `backend/docker-entrypoint.sh` runs `bootstrap-admin` at boot when `GRAS_BOOTSTRAP_ADMIN_EMAIL`/`_NAME` are set (refuses once an account exists; temp password once in the Render log; delete the vars after first sign-in). Measured at 512 MB / 0.1 CPU: boots in 112-199 s, under 150 MB, so a cold visit after 15 idle minutes waits 2-3 min.
- 2026-09-26 **Staging deploy tooling done** (branch `deploy/staging-render`): `render.yaml` Blueprint (Docker, `staging` branch, Frankfurt, `staging-api.`, secrets `sync: false`); the image now builds and runs the EF migration bundle on every boot (no deploy script on Render); `deploy/open-staging-db.sh` creates `gras_staging` only if missing, caps the role at 12 connections, TLS + listen address, a marked `hostssl` block and ufw rules replaced per run, refuses while ufw is inactive, checks the TLS cert before restarting the shared cluster (42/42 checks in an Ubuntu 24.04 container). `/code-review`: Render's outbound ranges are SHARED by every Render service in a region, so the address filter is not the boundary; the password plus `Channel Binding=Require` over TLS is. Health check is `/health/live` (Render restarts after 60 s of failures, and migrate-on-boot would loop on `/ready`); pool 5, role cap 12 for the deploy overlap. The staging image had **never been buildable**: floating `sdk:10.0` is the 4xx band that `global.json` (1xx, latestPatch) refuses (MCR stopped 1xx images at 10.0.103, now pinned), and `.editorconfig` was never copied, so CA1034 failed the build. Proven locally: image boots, 41/41 migrations over TLS with channel binding, health 200, second boot a no-op, 1.3 GB -> 534 MB (linux-x64 publish). Human 2026-09-26: `app.` is live on Netlify; `deploy-production` has 0 runs, so check whether Netlify auto-builds `main` ahead of the API. Still human: Render service, DNS for `staging-api.`/`staging.`, second Netlify site, push `main` to `staging`.
- 2026-09-25 **Status changes and transfers (spec 6.5.14, 6.5.16, 6.5.17) done**, branch `pupil-status-transfers`: `POST /pupils/{id}/status` (active to transferred/withdrawn closes the open enrolment on the effective date; active to graduated closes it at the session end; transferred/withdrawn/graduated to active reactivates into an arm of the active session with a new enrolment, number and history kept; a declined application cannot be reactivated; pending goes through the admissions queue), `POST /pupils/{id}/transfer` (same-session arm move; old enrolment closes the day before, per 6.4.4 step 3; `Enrolment.Transfer` corrected), `GET /pupils/{id}/enrolments`. Both POSTs take `dryRun` and return the consequences; privileges checked in the handler (`PupilRecordAccess`). Every non-Published result set of each affected arm is flagged; Awaiting Approval and Returned for Correction drop to Draft (Awaiting Approval carries "Cohort changed by pupil transfer on dd/MM/yyyy" in `returnReason`); Approved only gains the flag. **Human rulings 2026-09-25:** no future effective dates; a Published set of EITHER arm for the term the date falls in (or later) blocks a transfer; manual graduation allowed at any time. Status changes never block on or touch published sets. Reasons live on new table `pupil_status_changes` (migration `AddPupilStatusChanges`, additive), never in audit metadata. Capacity check shared with approval (`ArmCapacityGuard`). "Left during the term" no longer lists a pupil reinstated into the same arm. Frontend: "Class and status" tab on the pupil page with Move and Change status dialogs (dry run, then commit). Contract `757869b5…` +3 paths +9 schemas. Not built: bulk `POST /arms/{id}/transfer-in` (spec 06), the transfer log on the arm detail view. Next: files and photographs (6.5.8).
...
- 2026-09-25 **Status undo and dated graduation** (branch `status-undo`, human rulings same day): a manual graduation now takes an effective date and closes the enrolment on it (was the session end, which made a mistake irreversible until the session ended); `POST /pupils/{id}/status/undo` undoes the pupil's latest change when it is a leave recorded today (Lagos date), reopening the enrolment it closed (`Enrolment.Reopen`, the one exception to "effective_to set once"), re-flagging that arm's non-Published sets, and appending the undo as its own status-change row. A mistaken reactivation is not undoable this way. Frontend: "Undo this change" on the Class and status tab, graduation asks for a date. Contract `f7743971…` +1 path +1 schema. Review fixes: a leave's status-change row now records the arm it left and undo reopens only that enrolment (older rows without one are refused, not guessed); undo checks scope on that arm (an arm-scoped holder can undo their own withdrawal), requires the arm still Active and runs the capacity guard; one shared record+audit helper in `PupilMovementEngine`.
- 2026-09-25 **Frontend: change-password flow, Inter, dashboard sidebar** (branch `frontend-password-and-font`, human direction): `ProtectedLayout` holds a `mustChangePassword` account on a set-new-password form (the server 403s everything else) and `/account/password` offers a voluntary change, both on `POST /auth/password` (response replaces `me`); Inter via `@fontsource-variable/inter` (self-hosted, OFL) for body AND headings (the serif display face read as archaic); `AuthenticatedShell` is now a grouped sidebar (Pupils, Results, School setup, Administration; drawer below `lg`) with `lucide-react` icons (ISC, tree-shaken; first broad icon need), and gains the Admissions entry the old nav lacked. No contract change.
...
- 2026-09-23 **Bulk pupil import (spec 6.5.13) done**, branch `admissions-import` (stacked on `admissions-flow`): `GET /pupils/import/template` (XLSX via ClosedXML 0.105.0, MIT, behind `IPupilImportWorkbook`: header row, accepted values incl. the active session's arms, LGAs), `POST /pupils/import/validate` (writes nothing; every row accepted/rejected with column + reason; in-file duplicates rejected; register matches on surname + first name + DOB warned; capacity warnings) and `POST /pupils/import/commit` (the SAME file re-sent and re-validated statelessly, `fileSha256` must match, `skipRows`/`createRows` decide every match, `overrideCapacity` needs `arm.capacity.override`; all or nothing, one transaction). Rows build the same domain entities admission does; imported pupils go Active with numbers from `RegistrationNumberIssuer`, now the ONE issuance path shared with approval (one counter increment per row in file order, a number already held burns one serial, 3 draws then fail); each opens an enrolment; declaration unsigned; blank health = no health row (unanswered). Only Active arms of Active levels are importable. One bulk audit event (batch id, counts, per-year number ranges, file hash; no names). Idempotency fingerprint now includes multipart form fields. 1000 rows: validate ~1 s, commit ~6 s locally. Frontend `/pupils/import` (template, check, rejections table, skip/create, capacity override, CSV report, >500-row warning). Emergency primary contact NOT required on import (a responsible adult is), ruled by the human (next line). Contract `467ac28e…` +3 paths +8 schemas. Not built: the >1000-row background job, pickup/barred import (not importable by spec). Next: the incomplete-records report (done, above).
- 2026-09-23 **Admissions slice 2 (stepped flow, spec 6.5.11 + 6.5.16 override) done**, branch `admissions-flow` (stacked on `admissions-register`): `/admissions/:id?step=n` resumes at the first blocking step; steps 2-8 reuse the record panels, step 8 PATCHes section I only (nulls = unchanged), step 9 shows completeness + summary + the approve form; the queue gains New admission and Continue; approval confirmation shows the number large with Copy. New privilege `pupil.admission.override` (register 96; Super Admin + Head Teacher; migration adds it to Head Teacher additively in SQL, never overwriting an edited role); `healthOverrideReason` waives ONLY `health.unanswered`; the approval is audited as its own event, but the REASON is stored on `admission_records.health_override_reason` (migration `AddAdmissionHealthOverrideReason`), never in audit metadata, since it may describe the child's health; no read surface exposes it yet. Not built: admission slip print, the queue's per-row full missing list and last step reached, a leave-with-unsaved-changes guard. Next: bulk import (done, see above).
- 2026-09-23 **Human rulings for admissions slice 2+:** order is stepped flow, then bulk import, then status/transfers, then files/photo; parent-declined health questions are overridable at approval by a new `pupil.admission.override` privilege (Super Admin + Head Teacher by default) with a reason (>=10 chars), audited; **the `PrivilegeDecision` root fix is approved** (a SchoolWide grant satisfies `Unresolvable`; arm-restricted grants still refuse; full gate); bulk import is synchronous up to 1000 rows with a warning above 500, the 3000-row background job deferred.
- 2026-09-23 **Admissions slice 1 (spec 6.5.5-6.5.8, 6.5.12) done**, branch `admissions-register` (stacked on `weekly-reports-implementation`): `pupil_contact` (five roles, +234 phones, one primary adult), `authorised_pickup_person`, `barred_person_answer` + `barred_person`, `pupil_health` (nullable yes/no = never asked), `pupil_document` checklist (no file upload yet); 7 endpoints + `GET /admissions/{id}/completeness`; approval now 422 `admission.incomplete` naming each missing step. Health and barred reads are commands so their audit row commits; names and medical detail never enter audit metadata. **Privilege checks for pupil sub-records live in the handler (`PupilRecordAccess`)** because a route-level pupil scope cannot resolve a pupil with no open enrolment and refused even school-wide grants; `GET /pupils/{id}/weekly` moved to the same check. Frontend: pupil detail tabs Contacts / Collection / Health / Documents + completeness card with Go-to-step. Contract `9c7d96f3…` +7 paths +23 schemas. Next slice: stepped admission flow screen, document and photo files, bulk import, transfers/status.
- 2026-09-23 **Weekly reports (§6.10, appendix G) done end to end**, branch `weekly-reports-implementation`: `weekly_report` + five `weekly_report_day` rows + `arm_weekly_setting` (migration `AddWeeklyReports`, additive); weeks derived from term dates, never stored, a stored week that no longer matches is flagged `outsideTerm`, never deleted; 8 endpoints (arm grid with week list and per-account phrase memory, sparse PUT with required Idempotency-Key and last-write-wins, publish/unpublish refusing an empty week, auto-publish setting, pupil term view, completion report, illness summary behind `report.view` + `pupil.safeguarding.view`); auto-publish job at 17:00 Lagos Friday catching up to Sunday, once per arm-week so an unpublish sticks; portal term selector row + week list + day cards + appendix G PDF (cached per report revision, no use spent); frontend `/weekly` (by line with Fill across / Fill down / autosave on blur and every 30 s, by pupil) and `/weekly/completion`; `useClassChoice` promoted to `shared/pickers`. Contract `1e89286e…` +8 paths +21 schemas, additive verified by `jq`. `/code-review high` found 10, all fixed (unmount save lost mid-flight, closed-term `locked` flag, fill-down overwrote notes, manual unpublish overruled by auto-publish, last-edit taken from publish stamps, portal availability mismatch, N+1 lookups). Not built: head teacher dashboard marker (no dashboard yet).
- 2026-09-22 **CI on `origin2` (`fidelcomltd/gras_school_management_system`) — the deployment remote.** All three workflows are committed and registered **active** there, yet the repo has **0 runs ever**, and it holds **0 environments, 0 secrets, 0 variables** — so `deploy-production` cannot run at all: it requires a `production` environment plus `VPS_HOST` / `VPS_USER` / `VPS_SSH_KEY` / `VPS_KNOWN_HOSTS` (`backend/docs/DEPLOYMENT.md` §5). The orchestrator's token has **push but not admin/maintain** on that repo (repo-level Actions policy reads 403), so enabling Actions and adding the environment/secrets are **human-only** steps. Fixed on the way past, branch `ci/origin2-workflow-setup`: **`frontend-ci.yml` still triggered on `main` only** — the 2026-09-16 directive moved CI onto `staging` and changed backend-ci.yml alone, so every frontend gate, including `check:api-drift`, had been firing on `pull_request` and never on a `staging` push. **Local `staging` is 140 commits AHEAD of `origin/staging` and identical to `origin2/staging`**; the sync is a fast-forward the human must push, since the 2026-09-19 three-layer block (agent rule, `settings.json` deny, `pre-push` hook) refuses `refs/heads/staging` for every remote by design.
- 2026-09-22 **Production deployment readiness done** (branch `deploy/prod-readiness`, human's items 1-6): `Proxy` section + `UseForwardedHeaders` first in the pipeline (behind Nginx the rate-limit key, the portal's per-address pin limits and every audit `source_ip` otherwise collapse to 127.0.0.1, and HTTPS redirection loops); `DataProtection:KeyRingPath` persists the CSRF key ring (warns, not fails, when unset — blast radius is a `/auth/csrf` re-fetch); **TASK-0005b stage D done and PROVEN LIVE 2026-09-22** — `CloudinaryImageStore` (official SDK pinned 1.29.3, `type: authenticated`, `Overwrite=false`, credentials from env only, DI falls back to the in-memory fake and the validator refuses a half-configured host). Verified against the school's real account by the new `cloudinary-smoke-test` CLI subcommand (73-byte PNG, signed-URL read-back, byte-identical; refuses to pass on the fake store). **`AllowInMemoryStore` deliberately WINS over configured credentials**: the integration fixture runs as Development, `Program.cs` loads user-secrets there, so the first precedence (credentials first) would have pointed the whole integration suite at the live media library the moment a developer set user-secrets — caught 2026-09-22 when the human did exactly that; pinned by `SchoolImageStoreRegistrationTests`. The cost of that precedence is guarded in turn: `StartupEnvironmentGuard` now REFUSES to boot outside Development when `AllowInMemoryStore` is set (it ships `true` in the Development template, and a host carrying it over would accept the school's logo and lose it on the next restart, silently, with real credentials sitting right there), plus the integration fixture blanks the three credential keys so neither the flag nor a user-secret can put the suite on the network alone; `appsettings.Production.json`; and `deploy/` + `backend/Dockerfile` + `deploy-production.yml` + `backend/docs/DEPLOYMENT.md` (systemd, two Nginx hosts over one process, idempotent provisioning that generates the pin keys once, deploy = backup → install → migrate via same-commit bundle → symlink swap → restart → `/health/ready`, nightly verified dumps). Contract unmoved. **CI (Linux) then caught a defect no local run could: `KeyRingOptionsValidatorTests` asserted that BOTH path conventions validate on either platform, but `Path.IsPathRooted` answers for the running platform in both directions — `C:\...` is NOT rooted on Linux (it is a relative name there, and accepting it would create a directory literally called `C:\ProgramData\gras\keys` in the working directory). The production code was right; the test was wrong, and it passed on Windows because Windows roots both forms. Tests over `Path`/path strings must now assert per-running-platform, never "both everywhere".** `/code-review high` found 9, all fixed — 3 were first-deploy blockers (`source` truncating the connection string at its first `;`, `pg_dump --file` into a root-only directory, `mv -T` a symlink over a real directory). **Human 2026-09-22: the four hostnames are CONFIRMED** (`api.` / `results.` / `app.` / `staging-api.goldenroyalark.com`) **and the Cloudinary credentials are in local user-secrets.** Still needs the human: VPS IP + SSH access, DNS A records before certbot, Render region (staging DB allow-list), and an offsite backup destination (`/etc/gras/offsite.sh` warns on every run until then).
...
- 2026-09-06 **Admin-account audit events are LOG-ONLY**, not the transactional table 6.1.12
  ...
  Owner: `backend-dev`.*
- 2026-09-06 ~~**`mustChangePassword` is detected but there is still no change-password screen**~~ **STRUCK
  2026-09-25, branch `frontend-password-and-font`** — a flagged account is held on a set-new-password form on every
  protected route; `/account/password` for a voluntary change.
...
- 2026-09-23 **RULE FIXED 2026-09-23, WORKAROUND STILL IN PLACE (human ruling; `ScopeResolver` resolves an EXISTING pupil with no open enrolment to `RequiresSchoolWide`, an unknown id stays `Unresolvable`, so the 2026-09-18 result-set 403 ruling is untouched; `PrivilegeDecision` unchanged). No route declares `ScopeParameterKind.Pupil` yet, so the 8 `PupilRecordAccess` handler checks remain (they also give the 404 for an unknown pupil); a NEW pupil route should declare `ScopeParameterKind.Pupil`. Migrating the 8 is optional cleanup.** ~~`PrivilegeDecision` refuses a school-wide grant when a pupil-scoped route cannot resolve the pupil's arm~~ (pending admissions, leavers: `ScopeResolution.Unresolvable` is false for every grant). Worked around per route by checking in the handler (`PupilRecordAccess`, 8 routes now), which leaves `ScopeParameterKind.Pupil` without consumers and every future pupil route one forgotten call from refusing the office. The root fix, letting Unresolvable be satisfied by a SchoolWide grant, changes authorisation for every scoped route. *Trigger: the next pupil-scoped route, or a human ruling on the rule change (full gate required). Owner: orchestrator.*
- 2026-09-23 **RESOLVED 2026-09-23 (`testTimeout` 15 s, `asyncUtilTimeout` 3 s; `verify` 419/419 twice).** ~~Frontend `verify` flakes under full-suite load.~~ `bulk-create-arms-dialog.test.tsx` (and once each `admin-detail-screen`, pins, roles) misses its first default-1 s `findBy*` when the whole suite runs; alone it always passes, and consecutive full runs alternate 419/419 and 418/419. Not caused by weekly reports (no shared code). *Trigger: the next red `verify` that is not a real failure: raise the first query's timeout or cap vitest workers. Owner: orchestrator.*
- 2026-09-23 **RESOLVED 2026-09-23 (store now chosen at resolution from bound options; unit test reproduces the late-override order; scoped gate green with user-secrets present, no workaround).** ~~Local integration runs register the Cloudinary image store despite the fixture.~~ `InfrastructureDependencyInjection` reads `Cloudinary:*` at REGISTRATION time, before the test host's in-memory overrides apply, so with the school's credentials in user-secrets (since 2026-09-22) the fixture's `AllowInMemoryStore=true` and blanked keys arrive too late: `CloudinaryGateway`'s constructor throws "Cloud name must be specified" and every portal PDF test 500s locally (CI has no user-secrets, so it is green there). It fails closed, never reaching the network. Workaround used 2026-09-23: `$env:Cloudinary__AllowInMemoryStore='true'` for the run. *Trigger: next card touching the fixture or image store: set the flag as an environment variable in the fixture, or decide the store from bound options. Owner: orchestrator.*
