...
# NOT $APP_DIR itself: /opt/gras/api is a SYMLINK that deploy.sh points at the live release.
# Creating it as a real directory here is what made the first deploy fail after migrating, because
# `mv -T` refuses to replace a directory with a symlink.
install -d -o root -g root -m 0755 /opt/gras
...
# SINGLE-QUOTED because the value contains semicolons. systemd's EnvironmentFile parser strips the
# quotes; without them, anything that reads this file as shell truncates the value at the first `;`.
...
# `ufw allow` is idempotent, so the rules are simply asserted. A `--force reset` here would DELETE
# the hand-added "port 5432 from Render" rule (DEPLOYMENT.md section 7) on every re-run, which
# contradicts this script's own idempotence claim and would break staging silently.
